...
export async function generatePDFBuffer(data: SimulationData): Promise<Buffer> {
    ...
    drawJapaneseText(doc, "住宅ローン試算レポート", pageWidth / 2, yPosition, {
        align: "center",
    });

    yPosition += 6;
    doc.setFontSize(11);
    drawJapaneseText(doc, "サンワイデア株式会社", pageWidth / 2, yPosition, {
        align: "center",
    });
    ...
    drawJapaneseText(doc, `作成日: ${currentDate}`, pageWidth / 2, yPosition, {
        align: "center",
    });
    ...
    doc.setFontSize(12);
    doc.setFont(fontFamily, "bold");
    drawJapaneseText(doc, "希望借入金額", pageWidth / 2, yPosition + 5, {
        align: "center",
    });
    doc.setFontSize(16);
    ...
    drawJapaneseText(
        doc,
        formatManWithOku(data.result.wishLoanAmount),
        pageWidth / 2,
        yPosition + 11,
        { align: "center" },
    );
    ...
    drawJapaneseText(
        doc,
        formatManWithOku(monthlyPaymentValue),
        pageWidth / 2,
        yPosition + 11,
        { align: "center" },
    );
    ...
    doc.setFontSize(12);
    doc.setFont(fontFamily, "bold");
    drawJapaneseText(doc, "返済期間", pageWidth / 2, yPosition + 5, {
        align: "center",
    });
    doc.setFontSize(16);
    doc.setTextColor(44, 62, 80);
    ...
    if (data.usesBonus) {
        conditionRows.push([
            "ボーナス払い",
            `利用する（${formatManWithOku(data.bonusPayment ?? 0)}）`,
        ]);
    } else {
    ...
    if (data.hasLand) {
        conditionRows.push([
            "既存建築物",
            data.hasExistingBuilding ? "あり" : "なし",
        ]);
    } else {
        conditionRows.push([
            "土地予算の有無",
            data.hasLandBudget ? "あり" : "なし",
        ]);
    ...
    const specLabel = buildSpecLabel(
    ...
    drawJapaneseText(
        doc,
        "※ 本資料は無料のシミュレーション結果です。融資や契約の確定を示すものではありません。",
        pageWidth / 2,
        yPosition + 7,
        { align: "center" },
    );
    drawJapaneseText(
        doc,
        "※ プランイメージはあくまでサンプルです。実際の内容は担当者とご相談ください。",
        pageWidth / 2,
        yPosition + 13,
        { align: "center" },
    );
